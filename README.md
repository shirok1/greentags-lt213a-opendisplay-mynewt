...
图像直接流向屏幕 RAM，不分配 framebuffer。屏幕操作位于独立任务，BUSY 等待会让出 CPU，每阶段最多 30 秒；上电/关高压命令后先等待 10 ms，让 BUSY 有时间拉低。软件 SPI 每半位至少保持 1 µs。局刷五张 LUT 统一使用 100 帧驱动阶段，单次刷新；该参数来自 Zephyr 实板调校，Mynewt 的画质和时序仍需复测。
...
本次全局 `-O2` 构建（GCC 15.3.1）：应用 Flash **110,460 B**，静态 RAM **13,936 B**，中断栈 **384 B**，初始堆区 **2,064 B**。禁用未使用的 GATT indication/客户端过程池，保留 notification 和原有数据池，详见 [编译与裁剪说明](docs/power-saving.md)。

Cortex-M0 的任务栈参数按 32 位字计：主任务 384 字、屏幕任务 384 字、LL 任务 256 字。
构建生成 `.su` 栈使用报告；认证函数单独保留栈帧，避免把 AES 认证临时内存叠加到普通加密响应路径。实机栈高水位仍需测量。

Mynewt 原生默认已启用 30 秒硬件看门狗和 fatal 自动复位。本次把硬件看门狗显式配置为 **180 秒**，Mynewt 原生 idle task 仅在任务 sanity 检查通过后喂狗；主任务和屏幕任务均要求 **120 秒内有进展**，每 15 秒检查。空闲队列最长等待 60 秒；完成通知也计为进展，允许 MTU 23 下长配置读取，但卡住的通知等待不会喂狗。可调度的任务卡死约 120–135 秒触发 assertion 重启；调度/中断卡死由硬件看门狗兜底。Cortex-M0 原生 assertion/fault 路径已自带自动复位，本次固定关闭寄存器恢复调试和 coredump，不另写异常处理器。连接调试器时断言可能先触发断点，恢复测试须断开 SWD。
