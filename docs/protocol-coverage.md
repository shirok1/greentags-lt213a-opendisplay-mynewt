...
| `0043` | 版本 | 明文 0.2.0，12 字节构建 commit SHA |
| `0044` | MSD | 明文读取；按需采样芯片温度与内部 VDD ADC 电池电压，外设超时则 NACK |
...
- 原始 PIPE 全帧在最后一包 DATA 后自动发送 SACK、`00 82`，执行刷新并通知 `00 73` / `00 74`，与官方 SDK 对齐。压缩和局刷仍要求显式 END。重复 END 不会触发第二次刷新。
...
SecurityConfig 采用标准 64 字节记录；全零 master key 视为未启用。支持 session_timeout 和 rewrite_allowed；session_timeout 从认证成功开始计算总会话寿命，命令不会续期，0 表示不限制认证寿命；本板没有 reset GPIO，show-key-on-screen 是未提供的功能，相关 flags 会被拒绝。
...
- 60 秒窗口最多 10 次 challenge，已接受的第 10 次 challenge 仍可完成 proof；断开连接不会重置限流计数。
...
上电/断连后以 100–150 ms 参数快广播 30 秒，随后使用固定 1 秒慢广播参数。连接建立后或命令完成后开始计时，空闲 120 秒主动断连；仍有命令在处理时延后检查。
因此不能依赖空闲连接永久保留认证或 ETag 会话状态；长时间暂停的客户端应处理断连并重新认证/开始传输。

## 电池广播

CR2450 采用 lithium-primary 出厂配置；电压通过标准 MSD 字段发布（10 mV 单位），不新增 opcode 或电量百分比特征。启动、快广播结束及未连接时每 5 分钟后台采样；成功后更新广播数据，失败保留旧值，初次成功前为 0。`00 44` 仍按需采样。内部 VDD 作为电池电压代理，调试器供电和电路压降需实板校准；已有持久化配置不会被出厂配置覆盖。

## 传输绝对期限

Direct、PIPE 和局部会话从 START 起最多 15 分钟；数据包或其他命令不会延长期限。到期后取消图像、使 ETag 失效、清理配置事务及认证会话并断连。空闲时用现有 callout 在「120 秒空闲期限」与「剩余传输期限」中取较早者；忙时在命令边界检查，正在执行的有界屏幕刷新不被中途打断。完成的传输不再受旧期限影响。主机测试覆盖三类会话、计时回绕、到期清理及新 START；真实 BLE 的 120 秒断连和 15 分钟期限仍需上板计时验证。
