...
- 上电和断连后广播参数为 100–150 ms，30 秒后转 1–1.2 秒；发射功率仍为 0 dBm。慢广播可能延长发现时间。相较先前 1–1.2 秒 / 3–5 秒参数，这提高了发现和连接机会，也增加了广播耗电；实际续航需重测。
