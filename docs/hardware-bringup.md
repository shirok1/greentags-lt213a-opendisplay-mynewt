...
## 2026-09-12：复位后 LFRC 无进展修复

同板切换 Zephyr/Mynewt 实测：Mynewt 校准等待无进展，LFCLKSTAT=Running，但两个 RTC 与系统 tick 均为 0。仅重触发 CAL 无效；通过 SWD 先停止并重启 LFRC，再触发真实校准，RTC、调度与 BLE 恢复。没有伪造完成事件或跳过 PC。由此在 BSP 初始化时建立新的 LFRC 启动序列，并为启动等待加入约 1 秒 CPU 延迟上限，超时复位。具体底层硬件为何保留此状态尚未确定。

`tools/test_boot_hardware.py` 在原固件复现失败，时钟修复后连续 3 次软件复位均通过。此验证不是完整断电冷启动。切换 RTOS 后 macOS 缓存旧 GATT 句柄，导致订阅报 Writing is not permitted；功能基准使用仅测试构建覆盖的 BLE 地址，默认构建身份保持不变。

官方 py-opendisplay 7.14.1 的原始 PIPE 全帧测试另发现：固件只回复最后 DATA 的 SACK，SDK 等自动 END ACK，双方等待。协议现于原始全帧收齐后自动刷新；压缩和局刷继续等待显式 END。主机回归先复现失败，再验证修复，覆盖重排、序号回绕和重复 END 不重复刷新。
